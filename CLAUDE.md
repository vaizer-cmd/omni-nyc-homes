...
- **Short redirect `/1006`** (`vercel.json`, 302 route before the filesystem phase) sends visitors to a shared SharePoint folder. To change the target or add more short links, edit that route's `Location` header (`www.` also works — it first 307s to the apex domain).
...
**Key copy**: Home hero — "Your Building, Professionally Managed. Your Tenants, Always Connected."; tagline "Built on Trust, Driven by Excellence." Company is "OMNI Management LLC", founded by NYC real-estate veterans, serving all five boroughs. About/Contact/Services/Industry Expertise hero boxes are short — just the gold eyebrow label, no H1 (matches About's original style).
